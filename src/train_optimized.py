...
class EarlyStopping:
    """Para treinamento quando validação para de melhorar."""
...
def plot_learning_curves(log_path):
    ...
    plt.tight_layout()
    output_img = os.path.join(RESULTS_DIR, "curvas_aprendizado.png")
    plt.savefig(output_img, dpi=300)
    print(f"Gráficos salvos em '{output_img}'")
    plt.close()
...
def main():
    ...
    if DEBUG_MODE:
        print("MODO DEBUG: 1000 imagens, 5 épocas")
        NUM_EPOCHS = 5
        BATCH_SIZE = 32
    ...
    test_df.to_csv(os.path.join(RESULTS_DIR, "chexclusion_test_split.csv"), index=False)
    ...
    with open(log_file_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "train_loss", "val_loss", "train_auroc", "val_auroc", "lr"])
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"Loss - Treino: {epoch_loss_train:.4f} | Val: {epoch_loss_val:.4f}")
        if train_auroc > 0:
            print(f"AUC-ROC - Treino: {train_auroc:.4f} | Val: {val_auroc:.4f}")

        with open(log_file_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, epoch_loss_train, epoch_loss_val, train_auroc, val_auroc, optimizer.param_groups[0]['lr']])
